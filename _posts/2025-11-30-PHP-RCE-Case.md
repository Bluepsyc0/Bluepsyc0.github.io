...
**2. Next, the open ports discovered by the attacker are identified using the conversation view. In this view it is possible to see 8 ports that generated actual traffic, which indicates that these ports are open.**
...
**3. To determine the first port that was scanned, the traffic is filtered by the attacker’s IP and correlated with the timeline. By comparing the list of open ports with the earliest scanned port, port 22 is identified as the first one targeted.**
...
**5. The vulnerable PHP version shown in the final screenshot is 8.1.25, which falls into the range affected by the PHP CGI argument injection vulnerability CVE‑2024‑4577**
...
**6. From the same screenshot, it is also possible to see the username of the compromised user or service account under which the PHP process is running.**
...
**7. By analyzing all commands executed via RCE, the specific command that provided the attacker with an initial foothold on the system is identified, as shown in the screenshot.**
...
**8. This foothold can be mapped to a specific MITRE ATT\&CK technique ID, chosen based on the exploited CVE and the type of remote code execution used to gain access.**
